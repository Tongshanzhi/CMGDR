#!/usr/bin/env python3
"""
Run 4 new baselines (LGMRec, MENTOR, CausalRec, EliMRec)
with the same LOO protocol and sampled evaluation as CMGDR.
Appends results to the existing baseline_results.json.
"""
...
from src.models.baselines import LGMRecModel, MENTORModel, CausalRecModel, EliMRecModel
...
def run_baseline(model_class, model_name, config, seed, num_epochs=40, extra_kwargs=None):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in loader:
            ...
            # Model-specific auxiliary losses
            if isinstance(model, CausalRecModel):
                loss = loss + model.causal_loss(norm_adj, visual_features)
            elif isinstance(model, EliMRecModel):
                loss = loss + model.counterfactual_loss(norm_adj, visual_features, text_features)
...
def main():
    ...
    baselines = [
        (LGMRecModel, "LGMRec", {"n_hyper_clusters": 64}),
        (MENTORModel, "MENTOR", {"k_neighbors": 10, "n_coarse_clusters": 32}),
        (CausalRecModel, "CausalRec", {"adv_weight": 0.01}),
        (EliMRecModel, "EliMRec", {"cf_weight": 0.1}),
    ]
    ...
    # Print summary
    print(f"\n{'='*100}")
    print(f"NEW BASELINE RESULTS — Sports & Outdoors (LOO, Sampled 1+999)")
    print(f"{'='*100}")
    ...
    existing_path = Path(__file__).resolve().parent / "shared_data" / "evaluation" / "baseline_results.json"
    if existing_path.exists():
        existing = json.loads(existing_path.read_text())
        # Remove any old entries with same model names
        existing_names = {r["model"] for r in results}
        existing = [r for r in existing if r.get("model") not in existing_names]
        all_results = existing + results
    else:
        all_results = results

    out_dir = ensure_dir(Path(__file__).resolve().parent / "shared_data" / "evaluation")
    save_json(out_dir / "baseline_results.json", all_results)
    save_json(out_dir / "new_baseline_results.json", results)
    print(f"\nResults saved to: {out_dir / 'baseline_results.json'}")
    print(f"New results also saved to: {out_dir / 'new_baseline_results.json'}")
    ...
    if cmgdr_path.exists():
        ...
        combined = all_results + cmgdr_results
        lgcn_r10 = next((r["Recall@10"] for r in cmgdr_results if r["suffix"] == "LightGCN"), 0.1391)
        print(f"\n{'='*100}")
        print(f"FULL RANKING (all baselines + CMGDR)")
        print(f"{'='*100}")
        print(f"{'Method':<20} {'R@10':>7} {'R@20':>7} {'N@10':>7} {'N@20':>7} {'vs LightGCN':>12} "
              f"{'ExpGap':>8} {'CalGap':>8} {'Probe':>7}")
        print("-" * 100)
        for r in sorted(combined, key=lambda x: x.get("Recall@10", 0)):
            name = r.get("model", r.get("suffix", "?"))
            ratio = r["Recall@10"] / lgcn_r10 if lgcn_r10 > 0 else 0
            eg = r.get("exposure_gap@10", r.get("exposure_gap@10", 0))
            cg = r.get("calibration_gap@10", r.get("calibration_gap@10", 0))
            pr = r.get("probe_item_emb", r.get("probe_causal", 0))
            print(f"{name:<20} {r['Recall@10']:7.4f} {r['Recall@20']:7.4f} "
                  f"{r['NDCG@10']:7.4f} {r['NDCG@20']:7.4f} {ratio:11.2f}x "
                  f"{eg:8.4f} {cg:8.4f} {pr:7.3f}")
